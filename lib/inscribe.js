...
// Build BP07 single-step script to match Fractal/UniSat exactly: envelope 01 02 04 00 00 00 00 01 01 18 <24 bytes contentType> then bodyLen (1 byte) + body + OP_ENDIF.
function buildBp07TransferScript(ordinals, pubKey, addressType, inscription) {
  ...
  const pk = pubKey instanceof Uint8Array ? pubKey : (typeof pubKey === 'string' ? hex.decode(pubKey) : new Uint8Array(pubKey));
  const xOnlyPubkey = pk.length === 33 ? pk.subarray(1, 33) : (pk.length === 32 ? pk : pk.subarray(0, 32));
  const body = inscription.body instanceof Uint8Array ? inscription.body : new Uint8Array(typeof inscription.body === 'string' ? new TextEncoder().encode(inscription.body) : inscription.body || []);
  const bodyLen = body.length;
  if (bodyLen > 75) throw new Error('BP07 body must be ≤75 bytes');
  const contentType = (inscription.tags && inscription.tags.contentType) || inscription.contentType || 'text/plain;charset=utf-8';
  const contentTypeBytes = new TextEncoder().encode(contentType);
  const ctLen = Math.min(contentTypeBytes.length, 24);
  const ct24 = new Uint8Array(24);
  ct24.set(contentTypeBytes.subarray(0, ctLen), 0);
  // ord envelope: 00 63 03 6f 72 64 | 01 02 04 00 00 00 00 01 01 18 <24 bytes> | 00 (separator) | bodyLen(1) body | 68
  const ORD = new Uint8Array([0x00, 0x63, 0x03, 0x6f, 0x72, 0x64]);
  const ENV = new Uint8Array([0x01, 0x02, 0x04, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x18]);
  const rest = new Uint8Array(ORD.length + ENV.length + 24 + 1 + 1 + bodyLen + 1);
  let off = 0;
  rest.set(ORD, off); off += ORD.length;
  rest.set(ENV, off); off += ENV.length;
  rest.set(ct24, off); off += 24;
  rest[off++] = 0x00; // OP_0 separator between tags and body (indexer expects this)
  rest[off++] = bodyLen;
  rest.set(body.subarray(0, bodyLen), off); off += bodyLen;
  rest[off] = 0x68; // OP_ENDIF
  const bp07 = new Uint8Array(1 + 32 + 2 + rest.length);
  bp07[0] = 0x20;
...
function buildBp07RevealTx(commitUtxo, scriptHex, pubKey, privKey, toAddress, options = {}) {
  ...
  if (typeof keyPair.signSchnorr !== 'function') {
    throw new Error('BP07 reveal requires a signer with signSchnorr (use @bitcoinerlab/secp256k1 with ecpair).');
  }
