...
COMMANDS
  ...
  brc20-transfer  BRC-20 transfer (legacy or --single-step BP07; BP07 active on FB only)
...
`,
    'brc20-transfer': `
brc20-transfer — BRC-20 transfer (legacy or BP07 single-step)

  node lib/inscribe-cli.js brc20-transfer --tick <TICK> --amt <n> [--single-step] [--address-type N] [options]

  Ticker is case-sensitive; use the exact ticker from the token deploy (e.g. TheLonelyBit, not thelonelybit).
  --single-step  BP07: one commit→reveal sends to recipient (balance source = signer). Active on FB only, not BTC.
  --address-type 1..8. Default 1 (P2TR); with --single-step and --wif, defaults to 2 (P2WPKH) for bc1q/tb1q signer so indexer accepts. Use --out-order for reveal.
`,
    'brc20-mint': `
...
async function cmdReveal() {
  ...
  const singleStepScriptHex = commitRec && commitRec.singleStepScriptHex;
...
async function cmdBrc20Transfer() {
  ...
  const singleStep = hasArg('single-step');
  const wif = getArg('wif');
  const pubkeyHex = getArg('pubkey');
  // BP07 address type: 1=P2TR, 2=P2WPKH even, 3=P2WPKH odd. Indexer requires script to match signer; bc1q = P2WPKH → use 2 (or 3).
  let addressType = getArgNum('address-type', 1);
  if (singleStep && getArg('address-type') == null && wif) {
    try {
      ...
      if (derivedAddr && (derivedAddr.startsWith('bc1q') || derivedAddr.startsWith('tb1q'))) {
        addressType = 2; // P2WPKH; indexer expects OP_2 for single-step with native segwit
        process.stderr.write('Single-step: using address-type 2 (P2WPKH) for native segwit signer.\n');
      }
    } catch (_) { /* keep default 1 */ }
  }
  if (!tick || !amt) throw new Error('Need --tick and --amt');
  if (singleStep) {
    process.stderr.write('Note: Single-step transfer (BP07) is active on Fractal Bitcoin (FB) but NOT active on Bitcoin (BTC) mainnet. Use --chain fractal.\n');
  }
  // BRC-20 tickers are case-sensitive; wrong case (e.g. thelonelybit vs TheLonelyBit) can make transfers invalid when sending to others
  ...
  if (singleStep) {
    file.singleStepTransfer = true;
    file.addressType = addressType;
  }
  const files = [file];
  let pubKey, keyPair;
  if (wif) {
  ...
  if (orderPath) {
    ...
    if (keyPair) order.privKeyHex = Buffer.from(keyPair.privKey).toString('hex');
    order._recovery = {
      hint: 'For reveal: --commit-txid <TXID> --vout 0 --to <RECIPIENT> --order ' + orderPath + ' [--push]. WIF only needed when signing.',
      commitAddress: commitAddresses[0]?.address,
      requiredSats: commitAddresses[0]?.requiredAmount,
      recipientAtReveal: 'Set --to when you run reveal (e.g. bc1q...).',
    };
    fs.writeFileSync(orderPath, JSON.stringify(order, null, 2), 'utf8');
    process.stderr.write('Wrote order manifest: ' + orderPath + '\n');
    const addr = commitAddresses[0]?.address;
    const need = commitAddresses[0]?.requiredAmount;
    process.stderr.write('\nRecovery / next steps:\n');
    process.stderr.write('  • Save ' + orderPath + '. For reveal you need this file + commit txid + vout (0) + --to <recipient>. WIF only needed when signing reveal.\n');
    process.stderr.write('  • Send ' + need + ' sats to: ' + addr + '\n');
    process.stderr.write('  • Then: commit (if batching) or use this as outputs; sign-commit; reveal --commit-txid <TXID> --vout 0 --to <ADDR> --order ' + orderPath + ' [--push]\n');
  }
}
